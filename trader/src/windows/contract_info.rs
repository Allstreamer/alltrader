                    ...
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        ui.collapsing(&selected_contract.id, |ui| {
                            ...
                            ui.collapsing("terms", |ui| {
                                ...
                                ui.label(format!("deadline: {}",&selected_contract.terms.deadline));
                                ui.collapsing("payment", |ui| {
                                    // TODO: better date formatting
                                    ui.label(format!("on accept: {}",&selected_contract.terms.payment.on_accepted));
                                    ui.label(format!("on fulfill: {}",&selected_contract.terms.payment.on_fulfilled));
                                    ui.collapsing("deliver" , |ui| { 
                                        if let Some(goods) = &selected_contract.terms.deliver {
                                        for good in goods { 
                                            ui.collapsing(&good.trade_symbol, |ui| {
                                                ui.label(format!("destination: {}",good.destination_symbol));
                                                ui.label(format!("units required: {}",good.units_required));
                                                ui.label(format!("units fulfilled: {}",good.units_fulfilled));
                                            });
                                        }
                                    }
                                    });
